  ...
  return (
    <div style={{ position: "relative", width: 180, height: 180, margin: "0 auto" }}>
      ...
      <div style={{ position: "absolute", top: -8, right: -8, background: "#ef4444", color: "#fff", fontSize: 10, fontWeight: 800, padding: "4px 8px", borderRadius: 99, fontFamily: "monospace", letterSpacing: 1, boxShadow: "0 0 14px rgba(239,68,68,0.5)" }}
        className="pulse-red">CRITIQUE</div>
    </div>
  );
}
...
/* ── DATA ── */
const VULNS = [
  { article: "Art. 10", label: "Politique de divulgation des vulnérabilités", status: "critical", detail: "Absente ou non publiée — obligation légale CRA" },
  { article: "Art. 13", label: "Gestion des mises à jour de sécurité", status: "critical", detail: "Aucun processus documenté détecté" },
  { article: "Art. 11", label: "Certificat SSL/TLS", status: "warning", detail: "TLS 1.1 détecté — configuration faible" },
  { article: "Art. 15", label: "Documentation technique de sécurité", status: "critical", detail: "Aucune documentation publique disponible" },
  { article: "RGPD Art. 32", label: "Politique de confidentialité", status: "warning", detail: "Mentions incomplètes sur le traitement des données" },
  { article: "Art. 8", label: "Authentification et contrôle d'accès", status: "critical", detail: "Absence de 2FA sur interface admin détectée" },
];

const SCAN_LINES = [
  { text: "Initialisation du scanner CRA 2026...", delay: 200, color: "#64748b" },
  { text: "Résolution DNS + scan des ports exposés", delay: 700, color: "#64748b" },
  { text: "Vérification certificats SSL/TLS...", delay: 1200, color: "#64748b" },
  { text: "Contrôle politique de divulgation...", delay: 1800, color: "#f97316" },
  { text: "ALERTE — Article 10 CRA non conforme", delay: 2400, color: "#ef4444" },
  { text: "Analyse conformité Articles 8-15...", delay: 2900, color: "#64748b" },
  { text: "CRITIQUE — 4 violations majeures détectées", delay: 3500, color: "#ef4444" },
  { text: "Calcul du score de risque financier...", delay: 4100, color: "#64748b" },
  { text: "Rapport généré ✓", delay: 4600, color: "#22c55e" },
];

const CLIENTS = ["Stoïk", "Cyber Cover", "Vaadata", "Fidens", "Almond", "Hiscox", "AXA XL", "Coalition"];
      ...
      <div ref={mainRef} style={{ minHeight: "100vh", background: "#030308" }}>
        ...
        <nav className="h-nav" style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 clamp(20px,4vw,48px)", height: 60, background: "rgba(3,3,8,0.9)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(239,68,68,0.08)" }}>
          ...
          <div style={{ display: "flex", gap: 28, fontSize: 13, color: "#475569", fontFamily: "monospace" }}>
            {["Fonctionnement", "Articles CRA", "Partenaires"].map((n, i) => (
        ...
        {state === "form" && (
          <>
            ...
            <section style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "90px 24px 60px", textAlign: "center", position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", alignItems: "center", width: "100%" }}>
                ...
                <div className="h-badge" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 99, padding: "7px 18px", fontSize: 12, color: "#ef4444", marginBottom: 36, fontFamily: "monospace", letterSpacing: 1 }}
                  ...
                  ⚠️ AMENDES JUSQU'À 15M€ · ENTRÉE EN VIGUEUR DÉC. 2027
                </div>
                ...
                <div style={{ maxWidth: 860, marginBottom: 20 }}>
                  ...
                  <h1 className="h-t2" style={{ fontSize: "clamp(44px,7vw,86px)", fontWeight: 900, letterSpacing: "-2.5px", lineHeight: 1.0, marginBottom: 0 }}>
                    Votre produit<br />est-il conforme ?<br />
                    <span style={{ ...RED, backgroundSize: "200% auto", animation: "shimmer 4s linear infinite" }}>Score en 60s.</span>
                  </h1>
                </div>

                <p className="h-sub" style={{ fontSize: "clamp(15px,1.8vw,18px)", color: "#64748b", maxWidth: 520, lineHeight: 1.7, marginBottom: 44 }}>
                  Le CRA impose de nouvelles obligations à tous les fabricants de produits connectés vendus en Europe. Vérifiez votre exposition en 60 secondes.
                </p>

                {/* Stats */}
                <div className="h-stats" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 44, maxWidth: 560, width: "100%" }}>
                  {[
                    { v: "89%", l: "PME non conformes", c: "#ef4444" },
                    { v: "15M€", l: "amende max", c: "#f97316" },
                    { v: "Déc. 2027", l: "date limite", c: "#eab308" },
                  ].map((s, i) => (
                ...
                <div className="h-form" style={{ width: "100%", maxWidth: 520 }}>
                  <div style={{ padding: 2, borderRadius: 20, background: "linear-gradient(135deg,#ef4444,#dc2626,#b91c1c,#ef4444)", backgroundSize: "300%", animation: "gb 4s ease infinite" }}>
                    <form onSubmit={handleSubmit} style={{ background: "#0a0a12", borderRadius: 18, padding: "36px 32px" }}>
                      <div style={{ fontFamily: "monospace", fontSize: 12, color: "#ef4444", marginBottom: 20, letterSpacing: 2 }}>// OBTENIR MON SCORE CRA — GRATUIT</div>
                      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                        {[
                          { ph: "Nom de l'entreprise (ex: TechCorp SAS)", val: company, set: setCompany, type: "text" },
                          { ph: "URL du site (ex: https://techcorp.fr)", val: url, set: setUrl, type: "url" },
                          { ph: "Email professionnel", val: email, set: setEmail, type: "email" },
                        ].map((f, i) => (
                      ...
                      <button type="submit" style={{ width: "100%", marginTop: 20, background: "linear-gradient(135deg,#ef4444,#dc2626)", color: "#fff", fontWeight: 800, padding: "15px", borderRadius: 12, border: "none", cursor: "none", fontSize: 15, fontFamily: "inherit", boxShadow: "0 8px 28px rgba(239,68,68,0.35)", letterSpacing: 0.5 }}>
                        🔍 Analyser ma conformité — Gratuit
                      </button>
                      <p style={{ fontSize: 12, color: "#334155", marginTop: 12, fontFamily: "monospace", textAlign: "center" }}>Analyse 60s · Sans engagement · Confidentiel</p>
                    </form>
                  </div>
                </div>
              </div>
            ...
            <section style={{ padding: "80px 24px" }}>
              <div style={{ maxWidth: 820, margin: "0 auto" }}>
                <div className="sr" style={{ marginBottom: 56 }}>
                  <div style={{ display: "inline-block", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.15)", borderRadius: 99, padding: "5px 16px", fontSize: 12, color: "#ef4444", fontFamily: "monospace", letterSpacing: 1, marginBottom: 16 }}>// FONCTIONNEMENT</div>
                  <h2 style={{ fontSize: "clamp(28px,4.5vw,52px)", fontWeight: 800, letterSpacing: "-1.5px", lineHeight: 1.08 }}>
                    Trois minutes.<br /><span style={RED}>Zéro risque caché.</span>
                  </h2>
                </div>
                {[
                  { n: "01", i: "🔍", t: "Scan automatique", d: "Entrez l'URL de votre entreprise. Notre moteur analyse en temps réel vos expositions publiques — ports, SSL, politiques, documentation." },
                  { n: "02", i: "📊", t: "Score de conformité CRA", d: "Vous recevez un score sur 100 avec les articles violés, les preuves détectées et le risque financier estimé selon votre CA." },
                  { n: "03", i: "🛡️", t: "Plan de remédiation", d: "Notre expert certifié CRA vous contacte sous 24h avec un plan personnalisé. Votre assurance cyber reste valide." },
                ].map((s, i) => (
                  <div key={i} className="sr" style={{ display: "grid", gridTemplateColumns: "64px 1fr", gap: 24, padding: "32px 0", borderBottom: i < 2 ? "1px solid rgba(255,255,255,0.04)" : "none", alignItems: "flex-start" }}>
                    ...
                    <div>
                      <div style={{ fontSize: 10, color: "#ef4444", fontFamily: "monospace", letterSpacing: 2, marginBottom: 7 }}>ÉTAPE {s.n}</div>
            ...
            <section style={{ padding: "60px 24px 80px", background: "rgba(255,255,255,0.01)", borderTop: "1px solid rgba(255,255,255,0.04)" }}>
              <div style={{ maxWidth: 820, margin: "0 auto" }}>
                <div className="sr" style={{ marginBottom: 48 }}>
                  <div style={{ display: "inline-block", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.15)", borderRadius: 99, padding: "5px 16px", fontSize: 12, color: "#ef4444", fontFamily: "monospace", letterSpacing: 1, marginBottom: 16 }}>// CE QU'ON VÉRIFIE</div>
                  <h2 style={{ fontSize: "clamp(28px,4.5vw,52px)", fontWeight: 800, letterSpacing: "-1.5px", lineHeight: 1.08 }}>6 articles CRA.<br /><span style={RED}>Zéro tolérance.</span></h2>
                </div>
            ...
            <div style={{ padding: "24px 0", overflow: "hidden", borderTop: "1px solid rgba(255,255,255,0.04)", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
              <div style={{ fontSize: 11, color: "#334155", fontFamily: "monospace", textAlign: "center", marginBottom: 16, letterSpacing: 2 }}>PARTENAIRES ASSUREURS & COURTIERS</div>
              <div className="mq">
            ...
            <section style={{ padding: "80px 24px 100px", textAlign: "center" }}>
              <div style={{ maxWidth: 580, margin: "0 auto" }}>
                <div className="sr">
                  <div style={{ display: "inline-block", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.15)", borderRadius: 99, padding: "5px 16px", fontSize: 12, color: "#ef4444", fontFamily: "monospace", letterSpacing: 1, marginBottom: 20 }}>// PARTENARIAT ASSUREURS</div>
                  <h2 style={{ fontSize: "clamp(28px,4.5vw,52px)", fontWeight: 800, letterSpacing: "-1.5px", lineHeight: 1.08, marginBottom: 16 }}>
                    Devenez apporteur<br /><span style={RED}>d'affaires CRA.</span>
                  </h2>
                  <p style={{ color: "#64748b", fontSize: 17, lineHeight: 1.65, marginBottom: 36 }}>
                    Chaque client que vous nous référencez = commission 10-15%. Aucune charge opérationnelle pour vous.
                  </p>
            ...
            <div style={{ position: "relative", zIndex: 1, maxWidth: 560, width: "100%" }}>
              <div style={{ background: "rgba(10,10,18,0.95)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 18, padding: "36px 32px", backdropFilter: "blur(20px)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 24, fontFamily: "monospace" }}>
                  ...
                  <span style={{ fontSize: 12, color: "#ef4444", letterSpacing: 2 }}>SCAN EN COURS — {company || "ENTREPRISE"}</span>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ maxWidth: 760, margin: "0 auto" }}>
              ...
              <div style={{ textAlign: "center", marginBottom: 48 }}>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 99, padding: "7px 18px", fontSize: 12, color: "#ef4444", fontFamily: "monospace", letterSpacing: 1, marginBottom: 24 }}>
                  ...
                  RAPPORT DE CONFORMITÉ — {company.toUpperCase() || "VOTRE ENTREPRISE"}
                </div>
                <h1 style={{ fontSize: "clamp(32px,5vw,60px)", fontWeight: 900, letterSpacing: "-2px", lineHeight: 1.05, marginBottom: 8 }}>
                  Score <span style={RED}>critique.</span>
                </h1>
                <p style={{ color: "#64748b", fontSize: 16 }}>4 violations majeures détectées · Action immédiate requise</p>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
                <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(239,68,68,0.12)", borderRadius: 20, padding: "32px 24px", textAlign: "center" }}>
                  ...
                  <div style={{ marginTop: 20, fontFamily: "monospace", fontSize: 12, color: "#475569" }}>SCORE DE CONFORMITÉ</div>
                </div>
                <div style={{ background: "rgba(239,68,68,0.04)", border: "1px solid rgba(239,68,68,0.12)", borderRadius: 20, padding: "32px 24px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
                  <div style={{ fontSize: 12, fontFamily: "monospace", color: "#ef4444", letterSpacing: 1, marginBottom: 12 }}>💸 RISQUE FINANCIER ESTIMÉ</div>
                  <div style={{ fontSize: "clamp(24px,4vw,38px)", fontWeight: 900, color: "#ef4444", letterSpacing: "-1px", lineHeight: 1.1, marginBottom: 12 }}>250K€<br />— 2,5M€</div>
                  <p style={{ fontSize: 13, color: "#475569", lineHeight: 1.6 }}>Calculé sur CA estimé 1M-10M€. Le CRA prévoit jusqu'à <strong style={{ color: "#f1f5f9" }}>2,5% du CA mondial</strong> ou 15M€.</p>
                </div>
              </div>
              ...
              <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 20, padding: "28px", marginBottom: 16 }}>
                <div style={{ fontFamily: "monospace", fontSize: 12, color: "#ef4444", letterSpacing: 1, marginBottom: 20 }}>🔴 4 CRITIQUE · 🟡 2 AVERTISSEMENT</div>
              ...
              <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 20, padding: "28px", marginBottom: 24 }}>
                <div style={{ fontFamily: "monospace", fontSize: 12, color: "#ef4444", letterSpacing: 1, marginBottom: 20 }}>📊 DETTE SÉCURITÉ</div>
                {[
                  { l: "Conformité documentation", v: 15, c: "#ef4444" },
                  { l: "Sécurité infrastructure", v: 35, c: "#f97316" },
                  { l: "Gestion des vulnérabilités", v: 20, c: "#ef4444" },
                  { l: "Politique de mise à jour", v: 10, c: "#dc2626" },
                ].map((b, i) => (
              ...
              <div style={{ padding: 2, borderRadius: 20, background: "linear-gradient(135deg,#ef4444,#dc2626,#b91c1c,#ef4444)", backgroundSize: "300%", animation: "gb 4s ease infinite" }}>
                <div style={{ background: "#0a0a12", borderRadius: 18, padding: "36px 32px", textAlign: "center" }}>
                  <h3 style={{ fontSize: "clamp(20px,3vw,28px)", fontWeight: 800, marginBottom: 12, letterSpacing: "-0.5px" }}>
                    Télécharger le plan de remédiation
                  </h3>
                  <p style={{ color: "#64748b", fontSize: 15, marginBottom: 28, maxWidth: 460, margin: "0 auto 28px" }}>
                    Notre expert certifié CRA vous contacte sous 24h avec un plan personnalisé. Votre assurance cyber reste valide.
                  </p>
                  <a href="mailto:contact.zmimax@gmail.com" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "linear-gradient(135deg,#ef4444,#dc2626)", color: "#fff", padding: "15px 36px", borderRadius: 12, fontSize: 15, fontWeight: 700, textDecoration: "none", boxShadow: "0 8px 28px rgba(239,68,68,0.4)", fontFamily: "inherit" }}>
                    🛡️ Parler à un expert certifié CRA →
                  </a>
                  <p style={{ fontSize: 11, color: "#334155", marginTop: 14, fontFamily: "monospace" }}>Consultation 30 min gratuite · Devis sous 24h · Expert certifié</p>
                </div>
              </div>

              <p style={{ fontSize: 11, color: "#1e293b", textAlign: "center", marginTop: 24, fontFamily: "monospace" }}>
                Rapport basé sur une analyse passive des informations publiques. Première évaluation indicative — non un audit complet. ArmorCRA by Safwane.
              </p>
            </div>
          </div>
        )}
        ...
        <footer style={{ padding: "24px clamp(20px,4vw,48px)", borderTop: "1px solid rgba(255,255,255,0.04)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10, background: "rgba(0,0,0,0.4)" }}>
          ...
          <span style={{ fontSize: 12, color: "#334155", fontFamily: "monospace" }}>© 2026 · Par Safwane</span>
